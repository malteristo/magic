...
```bash
ssh turtle@100.110.46.104 "export PATH=/opt/homebrew/bin:\$PATH && tmux send-keys -t spirit-deep Enter"
```

### Enter pairing code

```bash
ssh turtle@100.110.46.104 "export PATH=/opt/homebrew/bin:\$PATH && tmux send-keys -t spirit-deep '/discord:access pair <CODE>' Enter"
```

### Health check

```bash
ssh turtle@100.110.46.104 "ps aux | grep 'bun server' | grep -v grep && echo '--- Bot is running ---' || echo '--- Bot is NOT running ---'"
```
