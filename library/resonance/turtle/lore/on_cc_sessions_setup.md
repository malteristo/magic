...
```bash
ssh turtle@[redacted-ts-ip] "export PATH=/opt/homebrew/bin:\$PATH && tmux send-keys -t spirit-deep Enter"
```

### Enter pairing code

```bash
ssh turtle@[redacted-ts-ip] "export PATH=/opt/homebrew/bin:\$PATH && tmux send-keys -t spirit-deep '/discord:access pair <CODE>' Enter"
```

### Health check

```bash
ssh turtle@[redacted-ts-ip] "ps aux | grep 'bun server' | grep -v grep && echo '--- Bot is running ---' || echo '--- Bot is NOT running ---'"
```
