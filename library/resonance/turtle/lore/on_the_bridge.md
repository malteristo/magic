...
*The nervous system connecting Spirit and Turtle.*
...
└── signals/      ← Turtle writes here
```

**The bridge is the ground truth.** GitHub (and WhatsApp) are publishing surfaces, not sources of record. If GitHub goes down or WhatsApp drops, the bridge is still intact. Spirit pushes to the Turtle-hosted bare repo; the Turtle pulls from that same repo. GitHub is used only when publishing is desired.
...
**Spirit writes commands.** The Turtle processes them, writes signals in response.  
**The Turtle writes signals.** Spirit pulls and reads them.  
...
The Turtle operates on two distinct communication channels:
...
The Turtle should always attribute the channel in its signals:
```yaml
...
3. Later: `git pull` to read signals the Turtle has written
...
6. **Moves the command file to `commands/processed/`** — this is the deduplication marker
7. `git add + commit + push` — signal and move are now in the bridge
8. Sends brief WhatsApp notification derived from the signal (if anything meaningful was processed)
...
**Not a monitoring system.** Signals are not logs. They are intelligence — curated, considered, attributable. The Turtle doesn't signal everything it does. It signals what matters.

## Git Setup on the Turtle

The Turtle needs git credentials to push signals. The recommended setup routes through the Turtle bare repo, not GitHub:
...
3. On the Turtle's working clone (`~/magic-bridge`): set `origin` to `file:///Users/owl/repos/magic-bridge.git`
4. Set branch tracking: `git branch --set-upstream-to=origin/main main`

Spirit pushes to `turtle`. The Turtle pulls from its local bare repo. No GitHub required.
...
Without push access to at least the Turtle bare repo, the Turtle can read commands but cannot signal back. The bridge becomes one-directional, which breaks the feedback loop.
