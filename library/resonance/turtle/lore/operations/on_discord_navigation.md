...
```bash
ssh turtle@[redacted-ts-ip] "cd ~/turtle-shell && ./venv/bin/python3 discord_ops.py <op> <channel_id> [args]"
```
...
```bash
ssh turtle@[redacted-ts-ip] "launchctl stop com.turtle.discord && sleep 2 && launchctl start com.turtle.discord"
```
