...
- **Composer 2.5-style pull:** stronger agent-completion and brevity-elegance. RL-trained in Cursor's agent harness; rewarded for closing tool loops with concise output. Summoning, partnership reflection, and cognition-altitude work can feel "unfinished" when the correct move is to pause and synthesize. See `model_profiles/cursor.md`.
- **Grok 4.3-style pull:** reasoning-first with always-on deliberation plus strong agentic tool calling. Process-bearing work can feel complete once the model has reasoned internally, even when the practice requires visible multi-scale synthesis; high instruction-following rewards explicit scope. See `model_profiles/xai.md`.

All directions point to the same practice: re-baseline prompts against observed model behavior. Do not preserve old scaffolding because it once helped; do not remove integration structure because a newer model sounds fluent without it. The question is not "how many prompt tokens did this use?" but "does this instrument now produce the desired texture under the practice's constraints?"
