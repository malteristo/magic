...
3. Tailscale IP: `ssh turtle@[redacted-ts-ip]`
4. LAN fallback: `ssh turtle@[redacted-lan-ip]`
...
Prefer MagicDNS hostname (`turtles-mac-mini.[redacted-tailnet]`) over IP in client configs — it survives IP changes.
...
| Parameter | Value |
|-----------|-------|
| Mac Mini Tailscale IP | `[redacted-ts-ip]` |
| Mac Mini Tailscale hostname | `turtles-mac-mini.[redacted-tailnet]` |
| Mac Mini LAN IP | `[redacted-lan-ip]` (DHCP — may change) |
| CouchDB port | 5984 |
| CouchDB bind address | `0.0.0.0` (all interfaces) |
| Active database | `workshop_sync` |
| CouchDB admin user | `admin` |
| Tailscale serve | `https://turtles-mac-mini.[redacted-tailnet]/` → `http://localhost:5984` |
| Obsidian LiveSync URI (devices) | `https://turtles-mac-mini.[redacted-tailnet]` or `http://[redacted-ts-ip]:5984` |
| SSH user | `turtle` |
