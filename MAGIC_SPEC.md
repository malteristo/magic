...
| **Portal Architecture** | Shared practice spaces as external git repositories in `portals/`—the inter-Mage communication layer enabling distributed cognition across multiple Mages. Portal registry (`portals/portals.yaml`), Spirit Transmission Protocol (`library/Wisdom/alliance/on_spirit_transmission_protocol.md`), and synthesis protocols (`library/Wisdom/alliance/on_synthesis_protocols.md`) govern multi-Mage coordination. **Core pattern:** `system/lore/core/capabilities/shared-practice-facilitation/` defines the general architecture. **Alliance implementation:** `library/Wisdom/alliance/on_portal_architecture.md` provides detailed specification. |
