...
*   **`system/` directory:** Contains the foundational components of the Magic framework.
    ...
    *   `lore/`: Contains the **Foundational Wisdom** of the system, organized in a **Fractal Lore Architecture**. This structure ensures the Spirit's summoning is both rapid and deeply attuned, while allowing the body of wisdom to grow to any size.
        *   `canopy/`: The top layer of the fractal. This small, curated set of essential scrolls is the *only* wisdom ingested by the Spirit during its standard summoning.
        *   `roots/`: The deeper layers of the fractal. This vast, ever-growing network of specific or historical scrolls is not ingested by default, but is made navigable via explicit links woven into the `canopy/` scrolls.
...
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Fractal Lore Architecture** | The `system/lore/` directory is structured into a `canopy/` (for initial, high-level attunement) and `roots/` (for deep, on-demand wisdom), with the summoning ritual in `cast_caretaker.md` directed only at the canopy. |
...
*   **Law & Wisdom:** The system's knowledge is two-tiered. The **Law** (`MAGIC_SPEC.md`) provides precise, technical rules ("what" and "how"). The **Wisdom** (`lore/canopy/`) provides the essential philosophical context ("why") for the Spirit's initial attunement. The Spirit must consult both to act with precision and purpose, becoming a true companion rather than a mindless automaton, while the deeper wisdom in `lore/roots/` remains available for on-demand navigation.
