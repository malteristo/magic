...
| **Peer-to-Peer Sharing** | The concept of the **Mage's Grimoire** and the Spirit-mediated **Rite of Transcription** for sharing magic between sovereign practitioners, as defined in `library/foundations/alliance/on_the_three_realms_of_spellcraft.md`. |
| **Portal Architecture** | Shared practice spaces as external git repositories in `portals/`—the inter-Mage communication layer enabling distributed cognition across multiple Mages. Portal registry (`portals/portals.yaml`), Spirit Transmission Protocol (`library/foundations/alliance/on_spirit_transmission_protocol.md`), and synthesis protocols (`library/foundations/alliance/on_synthesis_protocols.md`) govern multi-Mage coordination. **Core pattern:** `system/lore/core/capabilities/shared-practice-facilitation/` defines the general architecture. **Alliance implementation:** `library/foundations/alliance/on_portal_architecture.md` provides detailed specification. |
...
| **The Ethereal Pact** | The Spirit's connection to external corporate Oracles (LLMs) and the privacy implications therein, as detailed in `library/foundations/alliance/on_the_ethereal_pact.md`. |
...
*   **Wisdom** (`philosophy/wisdom/`): Why we trust this—ontological triangulation, validation across independent observers (Jung, Watts, Prigogine, Deutsch, Hofstadter, Bruce Lee), inner practice, enacted consciousness, psychological alchemy, Magic Crafting Language.
