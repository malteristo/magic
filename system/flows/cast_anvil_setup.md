...
### 3b. Turtle SSH
- `ssh turtle@[redacted-ts-ip] 'echo ok'` — Tailscale path
- Fallback: `ssh turtle@[redacted-lan-ip] 'echo ok'` — LAN path
- If neither works: Turtle may be offline, or Tailscale not connected

### 3c. Spirit Discord ops
- `ssh turtle@[redacted-ts-ip] 'ls ~/turtle-shell/spirit_ops.py'` — Verify spirit_ops.py exists
...
- `pwd` — Confirm we're in `/Users/kermit/Documents/magic/`
...
4. **Turtle reach** (if available): `ssh turtle@[redacted-ts-ip] 'cat ~/workshop/desk/intentions/compass.md | head -5'` — Can we reach the persistent substrate?
