...
**Areas to check:**
- All directories in `system/flows/`
- All directories in `library/resonance/` (including `resonance/foundations/`)
- All directories in `library/incubator/`
- `system/tomes/` only as archive: pointer map + deep-summoning variant, no live structure rules

## Structural Rules to Verify

### 1. Flow Structure (system/flows/*)

Every flow directory should have:
- [ ] `README.md` — Flow overview (can be brief)
- [ ] `cast_[name].md` — Primary spell (name should match directory) OR flow subdirectories that follow the same rule
