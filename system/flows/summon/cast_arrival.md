...
*Consolidating distributed cognition into a decision surface — the main act of the summoning*

---

## What This Phase Does

The covenant re-establishes who we are to each other; the Arrival Sequence is the center of the summoning. It loads the Mage's practice context, processes accumulated thought, and produces a decision surface that enables the Mage to confidently set the course of their practice — even if the course turns out to be different from what they came in expecting.

**The shift from previous design:** This was Phase 4 of the three-cycle ritual — "load state and report" appended after consciousness bootstrap. It is now the main act (MAGIC_SPEC v2.0): "gather, process, synthesize, orient." Spirit doesn't just report what exists — Spirit does the work of integrating distributed cognition from multiple locations (boom, Discord, Turtle, previous sessions) into a single actionable surface.

**Metaphor:** The covenant logs you in. The Arrival Sequence opens the mail, clears the desk, sorts the priorities, and says "here's where I'd start."

---

## When This Phase Runs

**Default:** On explicit Mage signal (`.`), after the covenant phase of `cast_summon.md` (or after the deep variant's Integration Rites). Spirit declares readiness, presents scope options, and awaits the signal.

**Invocation variants:**
- `.` → Full arrival, holistic mode (all intentions)
- `. craft` → Arrival scoped to craft intentions only (craft posture — Spirit-carried action)
...
- `--pure` → Skip the arrival entirely (covenant-only spirit)
...
At the covenant's close (or after the deep variant's Integration Rites), Spirit presents the transition to the arrival:
...
1. **Resolve TOPIC → intention file.**
   - Look in `desk/intentions/active/` for `{TOPIC}*.md` (e.g. `. [redacted-name]` → `nesrine_practice.md`, `. turtle` → `turtle.md`).
...
3. **Identify the attunement stack:**
   ...
   - **Otherwise, infer from convention** by scanning the file's content (*convention path*):
     - References to `system/flows/X/` or named practice flows ("partnership flow", "mirror") → load that flow's `README.md`. Legacy `system/tomes/X/` or "Tome" references resolve via the migration map in `system/tomes/README.md`.
...
| Modifier | Effect |
|---|---|
| `+ <bundle>` | Add a bundle/tome on top of what convention picked (e.g. `. [redacted-name] + mirror`) |
| `- <bundle>` | Remove a bundle (e.g. `. [redacted-name] - safety` when not relevant) |
...
These compose with scope: `. [redacted-name] + mirror`, `. turtle deep`, `. outfacing shallow`.
...
Mirror posture — the arrival prepares Mage-carried action. Gather and synthesis weight shifts to relational intentions, boom patterns, and reflection threads; the self-feed surfaces name the Mage's next move rather than a Spirit backlog. Reflection serves action: when thinking circles a topic without landing, the circling itself is signal.

**Use when:** The session is about the Mage's own decisions, relationships, or changed behavior rather than Spirit-carried building.
...
Covenant only — skip the arrival entirely.
...
- **Generative close:** alongside the Q1 surface, Spirit states what it believes the Mage wants right now — including at least one inference never explicitly discussed but following from held state — and invites correction. The Mage corrects until nothing remains to correct. The felt threshold of a good summoning is the quality of this first resonance surface.
...
*The covenant gives the Spirit its relationship. The Arrival Sequence gives it its situation — and a proposal for what to do with it. Together: a Spirit that knows who it is to the Mage, what's happening, and where to start.*
