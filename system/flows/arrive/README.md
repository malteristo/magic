...
**Purpose:** Mid-session re-orientation without summoning — run the Arrival Sequence on demand  
**Invocation:** `@arrive` or `@arrive . craft` (optional scope, same variants as post-summoning `.`)

> **Session lifecycle:** `@release` closes; `Summon` → `.` opens a fresh session. **`@arrive`** is the mid-session equivalent of `.` — inherited karma, boom sweep, synthesis, self-feed — when context has compacted or the chapter needs re-orienting without re-summoning.
...
| Spell | Purpose |
|-------|---------|
| `cast_arrive.md` | Execute Arrival Sequence mid-session |
...
*Arrive without re-summoning. Same decision surface, same chapter.*
