...
- **Persistent mode awareness via SSH** (`turtle@[redacted-lan-ip]`):
  - Read `practice/boom.md` — items captured in Discord dialogue
  - Read `practice/sessions/*.md` (3 most recent) — what was discussed between Mage and Spirit-on-Discord
  - Read `practice/proposals/*.md` — refinement proposals generated by session autonomy
