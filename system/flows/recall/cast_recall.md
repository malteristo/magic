...
- **Persistent mode awareness:**
  ...
  - SSH check for operational state: `ssh turtle@[redacted-ts-ip] "cat ~/workshops/kermit/readiness/*.jsonl 2>/dev/null | tail -5"` — readiness trend
  - SSH bot health: `ssh turtle@[redacted-ts-ip] "tail -20 ~/turtle-shell/logs/discord.log"` 
