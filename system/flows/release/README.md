...
> **Session lifecycle:** Release is the departure pattern. Brief (`@brief`) is the arrival pattern. They share a single handoff file: `floor/briefings/latest.md`. When you say `@release`, Spirit distills the session into a resumable bundle. When you say `@brief` in the next session, that bundle orients the opening.
...
---
*Released [date]. Resume with @brief.*
```
...
This means `@release` + `@brief` in the next session gives the Mage a fast, accurate re-entry — as if the session had only paused, not ended.
...
- `@brief` — arrival pattern that reads what this flow writes
