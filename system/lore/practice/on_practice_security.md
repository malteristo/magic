...
- **Names ship as mechanism, never as data.** The list of real people to keep out of public-bound files lives in gitignored `system/config/private_names.txt`. A checker that hardcodes a name publishes the name it guards, and protects exactly one household.
...
**Mitigation:** `./scripts/listener_audit.sh` — enumerate every non-loopback listener and compare it against `system/config/declared_listeners.txt`.
