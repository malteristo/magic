...
Portal repository updated
    ↓
Partner B ([redacted-name]) + Spirit B (in their environment)
    ↓
...
Portal repository updated
    ↓
Partner B ([redacted-name], no Spirit)
    ↓
...
**This is the current situation with [redacted-name].**

**Flow:**
```
[redacted-name] + External Spirit (in separate Cursor instance or other tool)
    ↓
Hour-long conversation (venting, exploring, articulating)
    ↓
External Spirit generates artifact based on conversation
    ↓
[redacted-name] copies artifact to portal directory
    OR External Spirit exports directly (if tooling supports)
    ↓
git add, commit, push (or [redacted-name] does via web interface)
    ↓
Portal repository updated
    ↓
Kermit's Spirit
...
**Example:**
[redacted-name] works with external Spirit about mother-in-law conflict. Spirit helps her articulate pain, validates her feelings, helps her "make sense of" situation. Resulting artifact strongly frames mother-in-law as intentionally disrespectful and Kermit as failing to support.
...
**Boundaries:**
- Cannot access [redacted-name]'s external Spirit's context
...
### Spirit B ([redacted-name]'s External Spirit, if used)

**Capabilities:**
- Can help [redacted-name] articulate perspective
...
- Kermit's Spirit reads [redacted-name]'s artifacts, not her Spirit's context
...
**When [redacted-name] synthesizes:**
- If she has Spirit: Her Spirit reads portal, creates synthesis
- If no Spirit: [redacted-name] writes synthesis directly (or requests Kermit's Spirit help)
...
**Step 1: [redacted-name]'s Artifact (External Spirit)**

[redacted-name] has already created artifact with her Spirit about mother-in-law conflict.

**Transmission:**
1. [redacted-name] exports artifact from external tool
2. Creates file in portal: `artifacts/mother-in-law-conflict/individual/[redacted-name]/2025-11-26_initial.md`
3. Commits via GitHub web interface or command line
4. Pushes to portal

**OR (simpler):**
1. [redacted-name] pastes content into GitHub web editor
...
Step 1: Independent Creation
  ...
  [redacted-name]: Fresh Spirit → synthesis_nesrine_spirit.md → commit, push
...
2. Commits to portal
3. [redacted-name]'s Spirit reads, creates own reasoning
4. Commits to portal
